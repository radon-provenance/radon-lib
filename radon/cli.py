...
class RadonApplication(object):
    ...
    def add_to_group(self, args):
        ...
        if added:
            self.print_success("Added {} to the group {}".format(", ".join(added),
                                                     group.name))
        if already_there:
            if len(already_there) == 1:
                ...
                verb = "are"
            self.print_error("{} {} already in the group {}".format(
                ", ".join(already_there),
                verb,
                group.name))
        if not_added:
        ...
        if args['<name>']:
            ...
            if user:
                ...
                if not user_info.get("ldap"):
                    ...
                    print("{0.bold}User name (ldap){0.normal}: {1}".format(
                            self.terminal,
                            user_info.get("username", name)))
                    print("{0.bold}Administrator{0.normal}: {1}".format(
                            self.terminal,
    ...
    def rm_from_group(self, args):
        ...
        if removed:
            self.print_success("Removed {} from the group {}".format(", ".join(removed),
                                                             group.name))
        if not_there:
            if len(not_there) == 1:
                verb = "isn't"
                ...
                verb = "aren't"
            self.print_error("{} {} in the group {}".format(", ".join(not_there),
                                                      verb,
                                                      group.name))
        if not_exist:
            if len(not_exist) == 1:
...
def main():
    ...
    arguments = docopt.docopt(__doc_opt__,
                       version='Radon Admin CLI {}'.format(radon.__version__))
