...
class TreeEntry(Model):
    ...
    container_acl = columns.Map(columns.Text, columns.UserDefinedType(Ace), static=True)
    ...
    def create_container_acl(self, acl_cql):
        ...
        query = SimpleStatement(
            u"""UPDATE tree_entry SET container_acl={} 
            WHERE container=%s""".format(
                acl_cql
            )
        )
    ...
    def create_entry_acl(self, acl_cql):
        ...
        session.set_keyspace(keyspace)
        query = SimpleStatement(
            u"""UPDATE tree_entry SET acl={} 
            WHERE container=%s and name=%s""".format(
                acl_cql
            )
        )
    ...
    def update_entry_acl(self, acl_cql):
        ...
        session.set_keyspace(keyspace)
        query = SimpleStatement(
            u"""UPDATE tree_entry SET acl=acl+{} 
            WHERE container=%s and name=%s""".format(
                acl_cql
            )
        )
